...
class LoRAManager:
    ...
    def initialize_adapters(
        ...
        *,
        ...
        initialization: str = "component-v1",
    ) -> Dict[str, SliceLoRA]:
    ...
    def set_trainable_parameters(self, arrays: Iterable[mx.array]) -> None:
        values = list(arrays)
        adapters = list(self._adapter_registry.values())
        if len(values) != 2 * len(adapters):
            raise ValueError("Expected one A/B parameter pair per adapter")
        # This setter also runs inside autodiff. Numeric validation belongs at
        # the optimizer commit boundary, outside the traced loss function.
        for index, adapter in enumerate(adapters):
            adapter.A = values[2 * index].astype(mx.float16)
            adapter.B = values[2 * index + 1].astype(mx.float16)
    ...
    def apply_pack(self, pack_dir: Path) -> PackMetadata:
        ...
        try:
            validate_base_identity(metadata, checkpoint_hash=self.base_hash, base_model=self.base_model)
        ...
        for key, rank in metadata.rank_map.items():
            ...
            for label, value in (("A", A), ("B", B)):
                if np.any(np.abs(value) > np.finfo(np.float16).max):
                    raise PackApplicationError(f"LoRA {label} on {key} exceeds finite float16 range")
            adapter = SliceLoRA(
                name=key,
    ...
    def export_active_pack(
        ...
        *,
        ...
        for key, adapter in self._adapter_registry.items():
            ...
            for label, value in (("A", export_A), ("B", export_B)):
                finite = mx.all(mx.isfinite(value) & (mx.abs(value) <= 65504.0))
                if not bool(finite.item()):
                    raise PackApplicationError(f"LoRA {label} on {key} must fit finite float16 storage")
        ...
        try:
            allowed_export_ranks = tuple(sorted(allowed_ranks_for(profile)))
        except ValueError as exc:
            raise PackApplicationError(str(exc)) from exc
        invalid_runtime_ranks = {
            key: rank
            for key, rank in rank_map.items()
            if rank not in allowed_export_ranks
        }
        if invalid_runtime_ranks:
            raise PackApplicationError(
                "Runtime active-rank allocation is not serializable for "
                f"profile '{profile}'; rebalance to allowed ranks "
                f"{list(allowed_export_ranks)} before export: {invalid_runtime_ranks}"
            )
