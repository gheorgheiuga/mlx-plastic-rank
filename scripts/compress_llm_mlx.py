...
def _cpu_stream() -> Any:
    ...
    if hasattr(mx, "stream"):
        stream_fn = cast(Any, mx.stream)
        return stream_fn(mx.cpu)
    return nullcontext()


def _device_stream(device: str) -> Any:
    if device == "gpu" and hasattr(mx, "gpu"):
        stream_fn = cast(Any, mx.stream)
        return stream_fn(mx.gpu)
    return _cpu_stream()
...
def compress_safetensors_file(
    ...
    *,
    ...
    if existing is None and out_path.exists():
        existing = load_file(str(out_path))
    ...
    for idx, name in enumerate(pbar):
        ...
        if arr.ndim == 2 and min(arr.shape) >= min_dim:
            ...
            if hasattr(arr_c, "__array__"):
                arr_c = np.array(arr_c, dtype=arr.dtype, copy=False)
            if local_device == "gpu":
                try:
                    mx.synchronize()
                    mx.clear_cache()
...
def main():
    ...
    ap.add_argument("--start-index", type=int, default=0, help="start tensor index (inclusive)")
    ap.add_argument("--end-index", type=int, default=None, help="end tensor index (exclusive)")
    ap.add_argument(
        "--batch-size",
        type=int,
        default=None,
        help="process tensors in batches of this size to control memory",
    )
    args = ap.parse_args()
    ...
    for f in tqdm(files, desc="Files", position=0):
        ...
        if batch_size is None:
            ...
            names = list(tensors.keys())
            total = len(names)
            end_limit = args.end_index if args.end_index is not None else total
            current = max(0, start)
            while current < end_limit:
                batch_end = min(current + batch_size, end_limit)
                changed, total = compress_safetensors_file(
                    f,
                    out_f,
                    target_energy,
